...
const SCHEMA_DOC = [
  ...
  '- chat.national(ts timestamptz, maturity, consommation, production, prevision_j, prevision_j1, nucleaire, hydraulique, pompage, eolien, solaire, gaz, fioul, charbon, bioenergies, ech_physiques, taux_co2)',
  "  Mesures nationales au quart d'heure depuis 2012. Valeurs numeriques (decimales) en MW ; taux_co2 en g/kWh ; ech_physiques negatif = la France exporte ; pompage negatif = stockage.",
  '  production = somme des filieres nucleaire + hydraulique + eolien + solaire + gaz + fioul + charbon + bioenergies, hors pompage et hors echanges : c est le denominateur de la part d une filiere (memes definitions que le brief du matin).',
  '- chat.regional(region_code, region_name, ts, maturity, consommation, production, thermique, nucleaire, eolien, solaire, hydraulique, pompage, bioenergies, ech_physiques)',
  '  Mesures des 12 regions metropolitaines, 24 mois glissants, MW ; production = thermique + nucleaire + eolien + solaire + hydraulique + bioenergies, hors pompage.',
  ...
  return [
    {
      ...
      content: [
        ...
        '- une part ou un pourcentage se calcule toujours en decimal : round(100.0 * solaire / production, 1), jamais une division entiere du type (solaire / production) * 100 qui vaut 0 ; la « part du solaire » sans autre precision est sa part de la production, la part dans la consommation se dit explicitement ;',
  ...
  return [
    {
      ...
      content: [
        ...
        'Les parts et pourcentages s arrondissent a une decimale avec une virgule (ex. 42,4 %).',
